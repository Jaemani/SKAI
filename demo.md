...
| 라이브(순수) | `scripts/demo.sh live` | **다중소스 연속 폴링**(OpenSky 항적 25s·GDELT 뉴스 5m·METAR 기상 30m·Celestrak 위성 12h) + 서버. **실데이터만, 합성 주입 없음.** 뉴스는 실 기사 URL로 클릭 가능. live 모드는 **군용 DB플래그 보강(adsb.fi /v2/mil 60s 스냅샷)·dropout 교차확인(adsb.fi)·LLM 서술 다듬기**가 기본 활성(각각 SKAI_MIL_ENRICH/SKAI_CROSSCHECK/SKAI_COPILOT_LLM, 미설정 시 자동 — DR-0013). 네트워크·API가 살아있을 때 오프닝 임팩트용. |
...
- **조작**: 지도만 보여준다(클릭 없음). KADIZ(파란 점선)·서해 작전구역 OpArea(주황 점선), 항적,
  위성 지상궤적(청록 점선), ADS-B gap 트랙(빨강 점선)이 이미 떠 있음. 우측 카운트 패널로 "항적·위성
  통과·기상·뉴스, 4소스가 지금 온톨로지에 들어와 있다"를 1초 짚음(숫자는 낭독하지 않음 — 화면이 증거).
...
- **신호 정직성(2026-07-05 DR-0013)**: 위성 근접 승격·상관은 **공개 문서화된 ISR-관련 위성
  허용목록(48기, Celestrak resource 실추출)만** — ISS 같은 정상 통과는 경고로 안 올라온다(스팸
  구조 차단). 상관 링크엔 **사유(시간차·공간관계)가 영속**돼 "왜 묶였는지"를 짚을 수 있다.
  예상 질문 방어: "허용목록 밖·기밀 정찰위성은 공개 TLE가 없어 못 잡습니다 — 그래서 위성 근접은
  단정이 아니라 정황(저신뢰 0.4)으로만 씁니다."
...
| # | 질의 | 예상 결과(재생·앵커 고정) |
|---|---|---|
| ① | 지금 KADIZ 근방 이상한 거 있어? | `해석` 창=**최근 30분(지금)**. 이상징후 **13건**·항적 16대. 문장 **54개** 전부 cites 보유. 종합 신뢰도 0.5. no_evidence=false. (실측 2026-07-05, 커밋 491568a 기준) |
| ② | 최근 1시간 위성 통과랑 겹치는 이상징후는? | `해석` 창=**최근 1시간**. 위성통과 상관 문장(correlated_with → SYN-RECON) 노출. 동일 13건. |
| ③ | 서해 쪽 기상이랑 뉴스 맥락 요약해줘 | `해석` 창=**최근 30분(기본)**. 하단에 기상(RKSI MVFR)·OSINT 뉴스(신뢰도 0.35, KADIZ 언급) 문장. `&sg=1`이면 서브그래프 자동 오픈. |

- **탐지 6유형·13건 확인**(실측 2026-07-05): 비상 스쿽(CES7500 0.95) · ADS-B dropout 3건(SHADOW1/
  SHADOW7 0.72 교차확인, GHOST2 0.42 미확인) · 로이터링(ORBIT3 0.60) · 군용기 접근 **5건**(RCH451/
  FALCON9/ROKAF31/PLAAF01 0.55, OLIVE21 0.50 — military_incursion 시나리오 3건 포함) · 위성 근접
  2건(SYN-RECON-1/7 0.40) · **급기동(ZOOM1 0.50)**. SHADOW7 상관 14건 전건 사유(시간차·거리) 보유.
- **군용기 프레임(필수·이중 경로 — DR-0013 #4·#5)**: 군용기는 ADS-B를 끄면 어떤 공개 피드에도
  안 보인다. 발표 문장 — "**트랜스폰더를 켠 군용기**는 콜사인·공개 DB 플래그로 저신뢰 식별하고,
  **끈 군용기**는 그 **부재(dropout)를 교차소스로 탐지**합니다. 군용 진입 시나리오는 '켠 채 진입'
  케이스의 결정루프 시연이며, 합성임을 화면 배지와 문장에 명시합니다." 절대 금지:
  ❌"군용기를 라이브로 전부 잡습니다"(콜사인·대역·DB플래그는 저신뢰 휴리스틱, 오탐·미탐 있음).
...
- **dropout 교차확인의 출처(정직)**: 재생의 0.72 "교차확인"은 **합성 미러**(결정성)에서 나온다.
  라이브 모드는 이제 **adsb.fi 교차확인·군용 DB플래그가 기본 활성**(DR-0013 — demo.sh live가 자동
  export). **인용 요건**: adsb.fi 크레딧은 UI 소스 패널에 상시 표기됨("Cross-check & mil data:
  adsb.fi") — 발화에서도 라이브 시연 시 한 번 언급하면 안전. 재생 모드엔 외부 호출 자체가 없다.
- **군용 라이브 가시성(신규)**: live 모드에서 군용 레이어는 ① adsb.fi 공개 DB 플래그(dbFlags&1,
  60s 스냅샷, 신뢰도 0.65) ② 콜사인·대역 휴리스틱(0.5~0.55)의 이중 근거로 채워지고, 판정 근거가
  mil_reason으로 설명문까지 전파된다. 오탐 실증 사례(민간기 DB 오등록)가 있으므로 "저신뢰"를
  항상 병기 — 단정 금지.
...
| 심사 항목 | 한 문장 | 화면 증거 | 수치 |
|---|---|---|---|
| **Problem Fit 25%** | 항적·위성·기상·뉴스가 흩어져 수작업 융합이 병목인 공중 상황인식을 정조준. | 스텝① 4소스가 한 지도·타임라인에 융합. | 소스 4종 동시 온톨로지 적재(재생 기준 항적 16기·이상징후 13건·문장 54개 전부 cites). |
