...
단위(네트워크·크리덴셜 불요): fake Foundry 어댑터를 주입해 HybridStore의
  - 라우팅(Aircraft·Observation·observed_as→Foundry, 나머지→Local)
...
def test_observed_as_link_routes_to_foundry(hybrid):
    store, fake = hybrid
    store.link("Aircraft", "abc123", "observed_as", "Observation", "abc123-1")
    assert fake.links and fake.links[0][2] == "observed_as"
    # 로컬 link 테이블엔 없음
