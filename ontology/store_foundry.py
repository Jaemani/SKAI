...
- **over(OrbitPass→Region)**: §15 E-2.1로 regionId FK 링크 형성(구 미형성 해소) → write_orbitpass의
  regionId가 이제 OrbitPass.region traverse 가능. **within(Observation→Region)**: 링크·create-
  observation.regionId 파라미터 모두 생겼으나(E-2.2) Observation 모델에 region_ref 필드가 없어 아직
  미배선(지오펜스 판정 로직 필요 — §15 잔여 갭).
...
- **Anomaly는 dual-write**(P7 §12-6 → §15 E부: 클린 실행 해소): 사용자가 create-anomaly 규칙의
  ...
  correlatedWith는 required지만 생성 시점 미상이라 present-only placeholder로 충족, **전체 근거·
  involves·correlated_with는 로컬 권위본**(링크 파라미터가 단수). confirm/dismiss 상태 전이는
  confirm-anomaly/dismiss-anomaly 액션으로 dual 동기.

## 스키마 잔여 이슈 (§15 E부 실측 갱신 — 코드로 못 고침, Ontology Manager UI 대응)
...
2. E-3 속성 추가됐으나 **채울 액션 파라미터가 없어 미배선인 것**: Observation.attrsJson(create-observation에
   attrsJson 파라미터 없음), OrbitPass.groundTrackJson(create-orbit-pass에 없음), WeatherState.station
   (create-weather-state에 없음 → weatherId PK 복원 유지), SituationAssessment.sentencesJson(create-
   situation-assessment에 없음 → 문장 cites 로컬 권위본 유지). Anomaly.createdAt·explainerBackend만
   create-anomaly에 파라미터가 있어 **배선 완료**(§15).
3. write_anomaly: §15 E부로 create-anomaly가 **클린 실행**(ApplyActionFailed 해소). _create_anomaly_
   absorbing은 방어용으로 유지(정상 경로 미발동). correlatedWith는 required지만 placeholder로 충족
   (correlated_with·다중 근거·involves 여러 건은 로컬 권위본, 파라미터 단수 한계).
4. set-region-alert-level은 OSDK 0.7.0에 **포함됨**(§15 실측, 0.5.0·0.6.0 누락 해소). 저수준 SDK로도 정상.
...
class FoundryOntologyStore:
    ...
    def write_orbitpass(self, orbit_pass: OrbitPass) -> None:
        ...
        #   그래프 traverse 가능. 잔여 갭: OrbitPass.groundTrackJson 속성은 생겼으나 create-orbit-pass에
        #   채울 파라미터가 없어(§15 실측) ground_track은 여전히 저장 안 됨(지도 궤적 레이어는 로컬 폴백).
        params = {
    ...
    def write_weatherstate(self, weather: WeatherState) -> None:
        ...
        #   station: E-3(§15)로 WeatherState.station 속성은 생겼으나 create-weather-state에 station
        #   파라미터가 없어(실측) 채울 수 없음 → 여전히 weatherId PK(f"wx-{station}-{ts}")로 복원.
        params = {
    ...
    def write_assessment(self, assessment: SituationAssessment) -> None:
        ...
        ⚠️ E-3(§15)로 SituationAssessment.sentencesJson 속성은 생겼으나 create-situation-assessment에
        sentencesJson 파라미터가 없어(실측) 문장별 cites(DR-0006 provenance 백본)는 여전히 저장 못 한다.
        권위본(문장 cites·aggregates/cites 링크)은 로컬이 보관하고(HybridStore가 dual-write), 여기서는
        스칼라 요약만 스파인으로 밀어 넣는다. create-situation-assessment의 anomalies·newsEvents·
        observations·orbitPasses(전부 여전히 required object)는 present-only placeholder로 충족.
        """
        params = {
    ...
    def write_anomaly(
        ...
        params: dict = {
            ...
            # correlatedWith = required(E-2.3 실측: Optional 아님, §15). 생성 시점엔 상관관계를
            #   모르고(write_anomaly 시그니처에 correlated 인자 없음) 다건 correlated_with는 로컬
            #   권위본이므로, present-only placeholder로 required만 충족(엣지는 안 맺힘, §15 실측).
            "correlatedWith": _ABSENT_REF,
        }
