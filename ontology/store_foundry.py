...
- **Anomaly는 dual-write**(P7 §12-6, D-1 부분 해소): create-anomaly가 evidenced_by 엣지를 안정
  형성하게 됐다(§12: `newParameter1` 제거로 6/6 형성). 단 EXECUTE가 매번 무해한 ApplyActionFailed
  (액션 후처리發, 데이터 무손상)를 던지므로 **catch 후 read-back(객체 존재+엣지)으로 성공 판정**한다.
  Foundry엔 스칼라 + 단일 observations(첫 근거)·단일 aircraft(첫 involves) 엣지만 밀고, **전체
  근거·involves·correlated_with는 로컬 권위본**(create-anomaly의 링크 파라미터가 단수라 나머지는
  Foundry로 못 감). confirm/dismiss 상태 전이는 confirm-anomaly/dismiss-anomaly 액션으로 dual 동기.
...
3. write_anomaly: create-anomaly가 evidenced_by 엣지는 안정 형성하나(§12: newParameter1 제거)
   매 EXECUTE마다 무해한 ApplyActionFailed를 던진다(액션 후처리發, 데이터 무손상) → catch 후
   read-back으로 흡수. correlated_with·다중 근거·involves 여러 건은 로컬 권위본(파라미터 단수 한계).
...
class FoundryOntologyStore:
    ...
    def write_anomaly(
        ...
        ## §12 에러 흡수 레시피 (핵심)
        create-anomaly는 valid refs로도 매 EXECUTE마다 무해한 ApplyActionFailed를 던진다
        (P7 §12-3: 액션 규칙 후처리에서 발생, 모든 edit 커밋 뒤라 데이터 무손상·non-atomic).
        → ApplyActionFailed를 catch하고 **read-back(Anomaly 객체 존재 + evidenced_by 엣지 형성)**
        으로 성공 판정한다. read-back이 실패할 때만 진짜 실패로 예외 전파.
        ...
        params: dict = {
            ...
            "newParameter": anomaly.id,  # anomalyId PK 바인딩
        }
