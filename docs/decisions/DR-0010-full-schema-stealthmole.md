...
4. StealthMole 연동 절차: API 정찰(엔드포인트·인증·리밋·ToS) → data-sources.md 갱신 → 커넥터(`connectors/stealthmole.py`) → NewsEvent(+mentions 링크). **키는 `~/SKAI/.env`에 `STEALTHMOLE_API_KEY`로**(gitignore), 코드·문서에 값 기재 금지.
