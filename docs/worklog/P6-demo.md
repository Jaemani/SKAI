...
- **AIP 캡스톤 사전조건(신규, 스텝 ⑥ 4번)**: `demo_foundry.sh`가 로컬 미러
  `data/demo/skai_foundry_local.db`에 skaidemo Anomaly+Region을 dual-write한 뒤, 코파일럿을 **`SKAI_STORE=foundry
  SKAI_COPILOT_LLM=aip SKAI_DB=data/demo/skai_foundry_local.db`** 환경으로 물려 "지금 KADIZ 근방 이상한 거
  있어?"를 던져 리허설한다. 확인: 응답 메타 **`store_backend=foundry`** · 헤드라인 **`produced_by=aip_logic`** ·
  **`overallAssessment`** 존재 · **문장 cites가 template 모드와 동일**(provenance 불변). AIP 함수 2종
  (`explain-anomaly`·`region-situation-summary`)이 OSDK(`.venv312`)에 존재하고 크리덴셜이 유효해야 실경로를
  탄다 — 실패하면 자동으로 `produced_by=template(aip 폴백)`으로 강등(응답은 나옴, 폴백 안전). **주의**:
  이 캡스톤은 `demo_foundry.sh`가 그 세션에 미러를 쓴 **직후**여야 자연 흐름으로 뜬다(disjoint 해소는
  db-regime.md §4). demo.sh엔 이 foundry-read 서버 모드가 없으므로 서버를 위 env로 직접 기동하거나 assess를
  직접 호출한다(범위 구분). 근거: `db-regime.md` §4 VERDICT AIP-NATURAL-OK.
