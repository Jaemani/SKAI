...
## 08. 온톨로지 서브그래프 (확장 상한 적용 — DR-0014)

![](screens/08_subgraph.png)

"지금 KADIZ 근방 이상한 거 있어?"(질의①) + `&sg=1` 딥링크로 연 서브그래프 — **실데이터**입니다.
단, 다른 화면과 달리 이 캡처는 상한 수정 검증을 위해 **별도의 후속 라이브 세션**(3사이클 누적,
이상징후 41건이 30분 창에 집계)에서 다시 찍었습니다 — 상단 안내문의 "26건" 스냅샷과는 다른 시각·
다른 창 값입니다(재현 불가능한 라이브 특성상 매 세션 값이 다름). 이전 버전은 이 창의 이상징후·
위성통과·상관을 전부 펼쳐
수백 노드 동심원이 되어 라벨이 겹쳐 판독 불가했습니다. 이번 캡처는 확장 상한(`build_subgraph`,
DR-0014)을 적용한 뒤라 **읽을 수 있는 그래프**입니다: aggregates(이상징후)는 신뢰도 내림차순
상위 8건, cites는 kind(위성통과/기상/뉴스/관측)별 상위 6건, correlated_with는 이상징후당 상위 3건만
그리고 초과분은 그룹당 회색 점선 **"+N건 더"** 표시자 노드 1개로 대체합니다(클릭하면 "전체 목록은
타임라인 참조"만 표시). 이 창에선 미표시 41→8건(+33건 더), 관측 인용 27→6건(+21건 더) 등으로
줄어 **전체 53노드·73엣지**(상한 적용 전 176노드·870엣지 추정)로 렌더됩니다 — 모달 상단 안내줄에도
"(상위 일부만 표시 — 정직 표기)"가 붙어 요약임을 숨기지 않습니다. 다만 correlated_with 상한은
이상징후 노드마다 개별 적용되므로, 교차상관이 많은 우측 클러스터는 여전히 "+N건 더" 마커가 여러 개
붙어 국소적으로는 다소 빽빽합니다 — 완전한 여백 확보보다 "헤어볼→읽히는 요약"으로의 개선입니다.
청록 점선은 `correlated_with`(교차 상관) 엣지, 실선은 `cites`/`evidenced_by`/`involves` 등
인용·근거 엣지 — 하단 범례가 노드 색·엣지 종류·"+N건 더" 표시자를 함께 설명합니다.
